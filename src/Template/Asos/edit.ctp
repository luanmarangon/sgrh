...
<div class="asos form large-9 medium-8 columns content">
    ...
    <fieldset>
        <legend><?= __('Editar Exames Ocupacionais') ?></legend>
        ...
        <div class="large-3 medium-4 columns">
            <?php
            $dtexame = substr($aso->dt_exame, -4)."-".substr($aso->dt_exame, 3, 2)."-".substr($aso->dt_exame, 0, 2);
            echo $this->Form->control('dt_exame', ['type' => 'text', 'label' => 'Data do Exame', 'value' => $dtexame]);
            ?>
        </div>
        <div class="large-3 medium-4 columns">
            <?php
        ...
        <div class="large-3  medium-4 columns">
            <?php
            $proximoexame = substr($aso->proximo_exame, -4)."-".substr($aso->proximo_exame, 3, 2)."-".substr($aso->proximo_exame, 0, 2);
            echo $this->Form->control('proximo_exame', ['type' => 'text', 'label' => 'Próximo Exame', 'value' => $proximoexame]);
            ?>
        </div>
        <hr>
        <div class="large-10 medium-12 columns">
            <?php
				if ($aso->anexo != ""){
					echo "<label> ASO cadastrado ";
					echo $this->Html->link(__('(clique aqui para substituir a imagem)'),  ['controller' => 'Asos', 'action' => 'excluiraso', $aso->id]);
					echo "</label>";
					echo $this->Html->image($aso->anexo);
